...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.dims = dims
        if use_conv:
            self.conv = u.conv_nd(dims, self.channels, self.out_channels, 3, padding=1)
...
class ResBlock(TimestepBlock):
    ...
    Args:
        ...
        use_conv: if True and out_channels is specified, use a spatial convolution instead of a
            smaller 1x1 convolution to change the
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            u.normalization(channels),
            nn.SiLU(),
            u.conv_nd(dims, channels, self.out_channels, 3, padding=1),
        )
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.norm = u.normalization(channels)
        self.qkv = u.conv_nd(1, channels, channels * 3, 1)
        ...
        self.proj_out = u.zero_module(u.conv_nd(1, channels, channels, 1))

    def forward(self, x):
        return u.checkpoint(self._forward, (x,), self.parameters(), self.use_checkpoint)
...
class UNetModelDiffusion(ModelMixin, ConfigMixin):
    ...
    Args:
        ...
        attention_resolutions: a collection of downsample rates at which attention will take place. May be a set,
            list, or tuple. For example, if this contains 4, then at 4x downsampling, attentionwill be used.
        ...
        num_heads_upsample: works with num_heads to set a different number of heads for upsampling. Deprecated.
        use_scale_shift_norm: use a FiLM-like conditioning mechanism.
        resblock_updown: use residual blocks for up/downsampling.
        use_new_attention_order: use a different attention pattern for potentially increased efficiency.
    """
    ...
    def __init__(
        ...
        self.time_embed = nn.Sequential(
            u.linear(model_channels, time_embed_dim),
            nn.SiLU(),
            u.linear(time_embed_dim, time_embed_dim),
        )
        ...
        self.out = nn.Sequential(
            u.normalization(ch),
            nn.SiLU(),
            u.zero_module(u.conv_nd(dims, input_ch, out_channels, 3, padding=1)),
        )

    def convert_to_fp16(self):
        """Convert the torso of the model to float16."""
        self.input_blocks.apply(u.convert_module_to_f16)
        self.middle_block.apply(u.convert_module_to_f16)
        self.output_blocks.apply(u.convert_module_to_f16)

    def convert_to_fp32(self):
        """Convert the torso of the model to float32."""
        self.input_blocks.apply(u.convert_module_to_f32)
        self.middle_block.apply(u.convert_module_to_f32)
        self.output_blocks.apply(u.convert_module_to_f32)

    def convert_to_bf16(self):
        """Convert the torso of the model to float32."""
        self.input_blocks.apply(u.convert_module_to_bf16)
        self.middle_block.apply(u.convert_module_to_bf16)
        self.output_blocks.apply(u.convert_module_to_bf16)
    ...
    def forward(self, x, timesteps, y=None, **kwargs):
        ...
        emb = self.time_embed(u.timestep_embedding(timesteps, self.model_channels))
...
class PatchedUNetCondCat(UNetModelDiffusion):
    """Patched UNet with conditioning upsampled and concatenated to input (see https://arxiv.org/abs/2207.04316).
