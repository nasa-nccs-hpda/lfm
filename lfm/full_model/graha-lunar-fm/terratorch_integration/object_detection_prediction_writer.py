...
class ObjectDetectionPredictionWriter(BasePredictionWriter):
    ...
    def __init__(
        ...
        print(
            f"ObjectDetectionPredictionWriter initialized. Saving to: {self.output_dir}"
        )
    ...
    def _extract_image_metadata(
        self, batch: Any, batch_indices: Optional[Sequence[int]], trainer: Any
    ) -> List[Dict[str, Any]]:
        ...
        if hasattr(trainer, "datamodule") and hasattr(
            trainer.datamodule, "predict_dataset"
        ):
        ...
        if not metadata_list:
            ...
            for i in range(batch_size):
                idx = (
                    batch_indices[i] if batch_indices and i < len(batch_indices) else i
                )
                metadata_list.append(
                    {
    ...
    def save_coco_format_file(self):
        ...
        coco_output = {
            "categories": [{"id": 1, "name": "IMP", "supercategory": "object"}],
