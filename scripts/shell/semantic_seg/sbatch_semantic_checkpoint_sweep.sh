#!/usr/bin/env bash
#SBATCH --job-name=sem_ckpt_sweep
#SBATCH --partition=compute
#SBATCH --gpus=1
#SBATCH --mem=128G
#SBATCH --cpus-per-gpu=10
#SBATCH --time=24:00:00
#SBATCH --output=scripts/logs/semantic_checkpoint_sweep_%j.out
#SBATCH --error=scripts/logs/semantic_checkpoint_sweep_%j.err
...
if [[ -f "${SUBMIT_DIR}/scripts/python/semantic_seg/semantic_checkpoint_sweep.py" ]]; then
  REPO_DIR="${SUBMIT_DIR}"
elif [[ -f "${SUBMIT_DIR}/../python/semantic_seg/semantic_checkpoint_sweep.py" ]]; then
  REPO_DIR="$(cd "${SUBMIT_DIR}/../.." && pwd)"
elif [[ -f "${SUBMIT_DIR}/../../../scripts/python/semantic_seg/semantic_checkpoint_sweep.py" ]]; then
  REPO_DIR="$(cd "${SUBMIT_DIR}/../../.." && pwd)"
else
  echo "Could not locate scripts/python/semantic_seg/semantic_checkpoint_sweep.py from submit directory: ${SUBMIT_DIR}" >&2
  exit 1
fi
