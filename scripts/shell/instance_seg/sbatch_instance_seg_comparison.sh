...
if [[ -f "${SUBMIT_DIR}/scripts/python/instance_seg/instance_seg_comparison.py" ]]; then
  REPO_DIR="${SUBMIT_DIR}"
elif [[ -f "${SUBMIT_DIR}/../python/instance_seg/instance_seg_comparison.py" ]]; then
  REPO_DIR="$(cd "${SUBMIT_DIR}/../.." && pwd)"
elif [[ -f "${SUBMIT_DIR}/../../../scripts/python/instance_seg/instance_seg_comparison.py" ]]; then
  REPO_DIR="$(cd "${SUBMIT_DIR}/../../.." && pwd)"
else
  echo "Could not locate scripts/python/instance_seg/instance_seg_comparison.py from submit directory: ${SUBMIT_DIR}" >&2
  exit 1
fi
...
python -u scripts/python/instance_seg/instance_seg_comparison.py "$@"
