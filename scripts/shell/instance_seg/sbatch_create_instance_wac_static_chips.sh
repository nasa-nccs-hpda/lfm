#!/usr/bin/env bash
#SBATCH --job-name=iseg_wac_static_chips
#SBATCH --partition=grace
#SBATCH --mem=192G
#SBATCH --cpus-per-task=16
#SBATCH --time=24:00:00
#SBATCH --output=scripts/logs/create_instance_wac_static_chips_%j.out
#SBATCH --error=scripts/logs/create_instance_wac_static_chips_%j.err
...
SCRIPT_REL="scripts/python/instance_seg/create_instance_wac_static_chips.py"

if [[ -f "${SUBMIT_DIR}/${SCRIPT_REL}" ]]; then
  REPO_DIR="${SUBMIT_DIR}"
elif [[ -f "${SUBMIT_DIR}/../python/instance_seg/create_instance_wac_static_chips.py" ]]; then
