"""Direct Graha/Lunar-FM true instance segmentation fine-tuning.

This script trains TerraTorch ``ObjectDetectionTask`` with ``framework="mask-rcnn"``
on Lunar WAC instance labels. It avoids the TerraTorch CLI while keeping the
same Graha/Lunar-FM backbone path used by the notebook smoke test.
"""
