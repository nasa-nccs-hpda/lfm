...
class PipelineTestCase(unittest.TestCase):
    ...
    # -------------------------------------------------------------------------
    # testInitWithTargetProductID
    # -------------------------------------------------------------------------
    def testInitWithTargetProductID(self):
        """Test initialization with targetProductID parameter"""
        outDir = Path(tempfile.mkdtemp())
        targetPID = "M1187363083CE"
        pl = Pipeline(self._tileDbPath, outDir, targetProductID=targetPID)

        self.assertEqual(pl._targetProductID, targetPID)
        self.assertEqual(pl._tileDbPath, self._tileDbPath)
        self.assertEqual(pl._outDir, outDir)

    # -------------------------------------------------------------------------
    # testInitWithoutTargetProductID
    # -------------------------------------------------------------------------
    def testInitWithoutTargetProductID(self):
        """Test initialization without targetProductID (defaults to None)"""
        outDir = Path(tempfile.mkdtemp())
        pl = Pipeline(self._tileDbPath, outDir)

        self.assertIsNone(pl._targetProductID)

    # -------------------------------------------------------------------------
    # testCreateCubeWithTargetProductIDFiltering
    # -------------------------------------------------------------------------
    def testCreateCubeWithTargetProductIDFiltering(self):
        """Test that createCube filters files when targetProductID is set"""
        tileDbPath = self.getTileDbPath(self._imageDir)
        outDir = Path(tempfile.mkdtemp())

        # First, run without filtering to see total
        pl_no_filter = Pipeline(tileDbPath, outDir, debug=False)
        ...
        layer_no_filter: ogr.Layer = pl_no_filter._query(ulLat, ulLon, lrLat, lrLon)
        prodIdDict_no_filter: dict = pl_no_filter._createCube(
            layer_no_filter,
            ulx, uly, lrx, lry,
            tileDef.srs,
            tileDef.tileWidth,
        ...
        # Now run with filtering for a specific product ID
        targetPID = "M1187363083CE"
        pl_with_filter = Pipeline(tileDbPath, outDir, debug=False,
                                targetProductID=targetPID)

        layer_with_filter: ogr.Layer = pl_with_filter._query(ulLat, ulLon, lrLat, lrLon)
        prodIdDict_with_filter: dict = pl_with_filter._createCube(
            layer_with_filter,
            ulx, uly, lrx, lry,
            tileDef.srs,
            tileDef.tileWidth,
        ...
        # With filtering, we should only have one product ID
        self.assertEqual(len(prodIdDict_with_filter), 1)
        self.assertIn(targetPID, prodIdDict_with_filter)

        # Without filtering, we should have many more
        self.assertGreater(len(prodIdDict_no_filter), 1)

        # The filtered result should have the same bands for that product
        self.assertEqual(
            len(prodIdDict_with_filter[targetPID]),
            len(prodIdDict_no_filter[targetPID])
        )

    # -------------------------------------------------------------------------
    # testCreateCubeStaticFilesNotFiltered
    # -------------------------------------------------------------------------
    def testCreateCubeStaticFilesNotFiltered(self):
        """Test that static files are NOT filtered by targetProductID"""
        tileDbPath = self.getTileDbPath(self._imageDir)
        outDir = Path(tempfile.mkdtemp())
        ...
        # Run with a target product ID for static files
        targetPID = "M1187363083CE"
        pl = Pipeline(tileDbPath, outDir, debug=False,
                    targetProductID=targetPID)
        ...
        if layer.GetFeatureCount() > 0:
            # Create cube with is_static=True
            prodIdDict_static: dict = pl._createCube(
                layer,
                ulx, uly, lrx, lry,
                tileDef.srs,
                tileDef.tileWidth,
            ...
            # Static files should NOT be filtered, so if there are static files
            # in this area, they should all be included regardless of targetProductID
            # (This test assumes there are static files; adjust based on your data)
            self.assertGreater(len(prodIdDict_static), 0)

    # -------------------------------------------------------------------------
    # testRunTileIndexWithTargetProductID
    # -------------------------------------------------------------------------
    def testRunTileIndexWithTargetProductID(self):
        """Test runTileIndex with targetProductID filtering"""
        tileDbPath = self.getTileDbPath(self._imageDir)
        outDir = Path(tempfile.mkdtemp())
        ...
        outFiles: list[Path] = pl.runTileIndex(tileX, tileY, zone, zoomLevel)

        # Should only have 1 WAC cube file (plus potentially 1 static file)
        # Check that all non-static files have the target product ID in the name
        wac_files = [f for f in outFiles if 'StaticCube' not in f.name]

        self.assertGreater(len(wac_files), 0)

        for f in wac_files:
            self.assertIn(targetPID, f.name)

    # -------------------------------------------------------------------------
    # testRunWithTargetProductID
    # -------------------------------------------------------------------------
    def testRunWithTargetProductID(self):
        """Test run() method with targetProductID filtering"""
        tileDbPath = self.getTileDbPath(self._imageDir)
        outDir = Path(tempfile.mkdtemp())
        ...
        outFiles: list[Path] = pl.run(ulLat, ulLon, lrLat, lrLon, zoomLevel)

        # Filter to only WAC files (not static)
        wac_files = [f for f in outFiles if 'StaticCube' not in f.name]

        # All WAC cube files should have the target product ID
        for f in wac_files:
            self.assertIn(targetPID, f.name)

        # Should have fewer files than without filtering
        self.assertLess(len(wac_files), 1782)  # Original test had 1782

    # -------------------------------------------------------------------------
    # testProductIDExtractionFromFilename
    # -------------------------------------------------------------------------
    def testProductIDExtractionFromFilename(self):
        """Test that product ID extraction logic works correctly"""
        # This tests the logic: fileName.stem.split('.')[0]
        from pathlib import Path

        # Test various filename formats
        test_cases = [
            ("M1187363083CE.ech.cog.tif", "M1187363083CE"),
            ("M1095664362CE.prj.uv.mos.tif", "M1095664362CE"),
            ("PRODUCT123.tif", "PRODUCT123"),
        ]

        for filename, expected_pid in test_cases:
            path = Path(filename)
            actual_pid = path.stem.split('.')[0]
            self.assertEqual(actual_pid, expected_pid,
                            f"Failed for {filename}: got {actual_pid}, expected {expected_pid}")

    # -------------------------------------------------------------------------
    # testNonMatchingProductIDSkipsFiles
    # -------------------------------------------------------------------------
    def testNonMatchingProductIDSkipsFiles(self):
        """Test that files with non-matching product IDs are skipped"""
        tileDbPath = self.getTileDbPath(self._imageDir)
        outDir = Path(tempfile.mkdtemp())

        # Use a product ID that likely doesn't exist
        targetPID = "NONEXISTENT_PRODUCT_ID"
        pl = Pipeline(tileDbPath, outDir, debug=True,  # Enable debug for skip count
                    targetProductID=targetPID)
        ...
        layer: ogr.Layer = pl._query(ulLat, ulLon, lrLat, lrLon)
        prodIdDict: dict = pl._createCube(
            layer,
            ulx, uly, lrx, lry,
            tileDef.srs,
            tileDef.tileWidth,
        ...
        # Should have no products since none match
        self.assertEqual(len(prodIdDict), 0)