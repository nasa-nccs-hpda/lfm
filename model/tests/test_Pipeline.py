...
class PipelineTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        imageDir = Path('/explore/nobackup/projects/ilab/' + 
                              'projects/Lunar_FM/' +
                              'data/testdata/NAC_PHO/NAC_PHO_E186N1213/WAC/')
        
        self._tileDbPath = self.getTileDbPath(imageDir)
    ...
    def testCreateCube(self):
        ...
        tileDbPath = self.getTileDbPath(imageDir)
        outDir = Path(tempfile.mkdtemp())
        ...
        self.assertEqual(layer.GetFeatureCount(), 32)
        
        cubeDict: dict = pl._createCube(layer, 
                                        ulx, 
                                        uly, 
                                        lrx, 
                                        lry, 
        ...
        cubeShape = (len(cubeDict), raster.shape[0], raster.shape[1])

        self.assertEqual(cubeShape, (32, 512, 512))
    ...
    def testQuery(self):
        ...
        layer: ogr.layer = pl._query(ulLat, ulLon, lrLat, lrLon)
        self.assertEqual(layer.GetFeatureCount(), 59)
    ...
    def testRunTileIndex(self):
        ...
        outFile: Path = pl.runTileIndex(tileX, tileY, zone, zoomLevel)
        
        ds = gdal.Open(outFile, gdalconst.GA_ReadOnly)
        self.assertEqual(ds.RasterXSize, 512)
        self.assertEqual(ds.RasterYSize, 512)
        self.assertEqual(ds.RasterCount, 32)
    ...
    def testRunPoint(self):
        ...
        outFile: Path = pl.runPoint(lat, lon, zone, zoomLevel)
